...
struct midi_stream_t
{
     ...
     inline  midi_stream_t() { reset(); }
     inline ~midi_stream_t() {}

     inline void add( const size_t time_tics, size_t size, uint8_t* data )
       {
     ...
     inline void sort_and_merge()
       {
          // сортировка
          items.sort([](event_item_t const& a, event_item_t const& b)->bool {return a.time_tics < b.time_tics; }) ;
    ...
    inline void reset() { curr = items.begin(); }
