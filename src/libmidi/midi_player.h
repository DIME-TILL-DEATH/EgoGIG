...
struct midi_player_t
{
  ...
  inline void pos(size_t val)
    {
       ...
       while( midi_stream.curr->time_tics < sample_count && midi_stream.curr != midi_stream.items.end())
	 {
	   midi_stream.curr++ ;
	 }
    }
	  ...
	  while ( midi_stream.curr->time_tics < sample_count )
	      ...
	      // настройка dma
              ...
              if (  !(DMA_SCR(DMA2, DMA_STREAM7) & DMA_SxCR_EN) )
        	{
              ...
              midi_stream.curr++ ;
              if ( midi_stream.curr == midi_stream.items.end()) break ;
           }
       }
  ...
  inline void reset() { midi_stream.reset() ; sample_count = 0 ;}
