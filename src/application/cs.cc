...
uint8_t menu_list   [][16] = {"Select Playlist","Edit Playlist","System","Set MIDI Ctrl"};
uint8_t system_list [][11] = {"Auto next ","Count dir ","Scroll dir","LB points "};
uint8_t off_on      [][5]  = {"Off ","On  "};
uint8_t up_down     [][5]  = {"Up  ","Down"};
uint8_t contrl_list [][12] = {"Play ","Pause","Stop ","Channel","MIDI PC Set"};
uint8_t note_list   [][3]  = {"C ","C#","D ","D#","E ","F ","F#","G ","G#","A ","A#","B "};
uint8_t pc_list     [][5]  = {"PC  ","CC  ","Note"};

uint8_t  num_menu = 0;
uint8_t  num_prog = 0;
uint8_t  num_prog_edit = 0;
const uint8_t led_sym[10] = {0xc0,0xf9,0xa4,0xb0,0x99,0x92,0x82,0xf8,0x80,0x90};
extern volatile uint8_t encoder_state,encoder_state1,encoder_key,key_ind,play_fl,play_fl1;
...
inline void check_busy(uint8_t type, uint8_t val , uint8_t num , uint8_t id_comm)
{
	...
	if(!id_comm)
	{
		for(uint8_t i = 0 ; i < 3 ; i++)
		{
			if(val == ctrl_param[i * 2 + 1] && num != i)
			{
				...
				{
					if(type == ctrl_param[j * 2])DisplayTask->StringOut(11,1,(uint8_t*)"Busy");
				}
			}
		}
		for(uint8_t j = 0 ; j < 99 ; j++)
		{
			...
			{
				if(type == pc_param[j * 2])DisplayTask->StringOut(11,1,(uint8_t*)"Busy");
			}
		}
	}
	/*for(uint8_t i = 0 ; i < 99 ; i++)
	{
		if(num != i)
		{
			if(val == pc_param[i * 2 + 1])
			{
				if(type == (pc_param[i * 2] + 1 ))DisplayTask->StringOut(11,1,(uint8_t*)"Busy");
			}
		}
	}*/
/*	else {
...
inline void string_lin(uint8_t col , uint8_t page , uint8_t val)
{
	...
	if(line_buf[0] == 48)line_buf[0] = 32;
	line_buf[1] = val % 100 / 10 + 48;
	if(line_buf[1] == 48 && line_buf[0] == 32)line_buf[1] = 32;
...
inline void midi_note_print(uint8_t val)
{
	...
	if(val < 12)DisplayTask->StringOut(7,1,(uint8_t*)"-1");
	else {
...
{
	switch(type){
	case 0:DisplayTask->StringOut(0,1,(uint8_t*)"CC  ");break;
	case 1:DisplayTask->StringOut(0,1,(uint8_t*)"Note");break;
...
{
	switch(type){
	case 0:midi_type_print(type);string_lin(5,1,val + 1 );DisplayTask->StringOut(5,1,(uint8_t*)line_buf);break;
	case 1:midi_type_print(type);midi_note_print(val);break;
	}
}
...
void jump_rand_pos(uint32_t pos)
{
  ...
  if(sys_param[direction_counter])DisplayTask->Sec_Print(count_down);
  else DisplayTask->Sec_Print(count_up);
...
inline void init_prog(void)
{
  ...
  FsStreamTask->pos(0);
  msec_tik = 0;
  for(uint32_t i = 0 ; i < (wav_buff_size) ; i++)sound_buff[i].left = sound_buff[i].right = click_buff[i].left = click_buff[i].right = 0;
}
uint8_t load_prog(void)
{
  while(!play_fl1);
  if(FsStreamTask->open(num_prog))return 1;
  else {
      DisplayTask->Clear();
      emb_string tmp ;
...
uint8_t test_file(void)
{
  uint8_t temp = 0;
  no_file = 0;
  for(; num_prog < 100 ; num_prog++)if(!load_prog())break;
  if(num_prog > 98)
  {
    num_prog = 0;
    temp = 1;
...
inline void init_play_menu(uint8_t type)
{
    ...
    if(stop_fl1 && !type)
    {
        if(!test_file())if(num_prog < 99)DisplayTask->Sec_Print(FsStreamTask->sound_size());
    }
    else {
        DisplayTask->Clear();
    ...
    key_reg_out[0] |= 0x10;
    key_reg_out[0] &= ~0x8;
    if(play_point1_fl)key_reg_out[1] &= ~(1 << 7);
    if(play_point2_fl)key_reg_out[1] &= ~(1 << 15);
}
/////////////////////////////////////////////Gui//////////////////////////////////////////////////////////////
...
//------------------------------------Player----------------------------------------------
	case player:
	 if(pause_fl)
	 {
	    if(tim5_fl)key_reg_out[0] |= 2;
	    else key_reg_out[0] &= ~2;
	 }
	 ...
	 if(key_ind == key_start && !no_file)
	 {
	    ...
	    if(!play_fl)
	    {
	        ...
	        key_reg_out[0] |= 2;
	        key_reg_out[0] &= ~0x80;
	        if(play_point1_fl && !play_point_ind && !pause_fl && sys_param[loop_points])jump_rand_pos(play_point1);
	        pause_fl = 0;
	        us_buf1 = 0xfa;
	        MIDITask->Give();
	 ...
	 if(key_ind == key_return && !no_file)
	 {
	    if(ret_dub_fl)
	    {
	       ...
	       play_point1_fl = 1;
	       if(!play_point2)play_point2 = song_size * 4410;
	       key_reg_out[1] &= ~(1 << 7);
	    }
	    else jump_rand_pos(play_point1);
	    clean_fl();
	 }
	 if(key_ind == key_forward && !no_file)
	 {
	    ...
	    else jump_rand_pos(play_point2);
	    clean_fl();
	 }
	 if(key_ind == key_left_down)
	 {
	  ...
	  {
	    dela(0xfffff);
	    if(num_prog > 10)num_prog -= 10;
	    while(1)
	      {
		if(load_prog())
	        {
		   if(num_prog)num_prog--;
		   else num_prog = 100;
	        }
		else break;
	      }
	 ...
	 if(key_ind == key_left_up)
	 {
          if(stop_fl1)
	  {
            ...
            while(1)
	     {
            	if(load_prog())num_prog = (num_prog + 1) % 99;
            	else break;
	     }
	    load_led(num_prog);
	    clean_fl();
	 ...
	 {
	    if(stop_fl1)stop_fl1 = 0;
            if(encoder_state == 2)
	    {
              if(count_up < song_size)count_up = enc_speed_inc(count_up, song_size);
	      count_down = song_size - count_up;
	      if(sys_param[direction_counter])DisplayTask->Sec_Print(count_down);
	      else DisplayTask->Sec_Print(count_up);
	      ...
	      memset(click_buff,0,wav_buff_size);
	      sound_point = 0;
	      if(play_fl2)play_fl = 1;
	    }
            else {
	      if(count_up)count_up = enc_speed_dec(count_up,0);
	      count_down = song_size - count_up;
	      if(sys_param[direction_counter])DisplayTask->Sec_Print(count_down);
	      else DisplayTask->Sec_Print(count_up);
	      FsStreamTask->pos(count_up * 4410);
	 ...
	 if(key_ind == key_encoder && !no_file)
	 {
		 ...
		 {
		     if(!FsStreamTask->open_song_name(prog_temp,tmp,0,0))break;
		 }
		 if(prog_temp != 100)
		   {
		     oem2winstar(tmp) ;
		 ...
		 else DisplayTask->StringOut( 2 , 0 , (uint8_t*)"Playlist End");
		 Delay(1500);
		 condish = player;
		 DisplayTask->Clear();
		 FsStreamTask->sound_name(tmp);
...
//------------------------------------Menu------------------------------------------------
	case menu:
	  if(tim5_fl)DisplayTask->Clear_str(0, num_menu & 1 , 15);
	  else DisplayTask->StringOut(0, num_menu & 1 , (uint8_t*)menu_list + num_menu * 16);
	  if(encoder_state1)
	  {
		  if (encoder_state == 1)
		  {
			  if(num_menu)
			  {
				  if(num_menu == 2)
				  {
					  ...
					  DisplayTask->StringOut(0, num_menu & 1 , (uint8_t*)menu_list + num_menu-- * 16);
					  tim7_start(0);
				  }
				  DisplayTask->StringOut(0, num_menu & 1 , (uint8_t*)menu_list + num_menu * 16);
				  clean_fl();
			  ...
			  if((num_menu < 3) && stop_fl1)
			  {
				  if(num_menu == 1)
				  {
					  ...
					  num_menu++;
					  DisplayTask->StringOut(0, 1 , (uint8_t*)menu_list + 3 * 16);
					  tim7_start(1);
				  }
				  else {
					  DisplayTask->StringOut(0, num_menu & 1 , (uint8_t*)menu_list + num_menu++ * 16);
	  ...
	  if(key_ind == key_encoder)
	  {
		...
		case 1:{
		  ...
		  else DisplayTask->StringOut(2,0,(uint8_t*)"  No wav file");
	      }break;
	      case 0:{
		  if(stop_fl1)
		  {
		     ...
		     DisplayTask->StringOut(0,1,(uint8_t*)"Playlist folder");
		     Delay(1000);
		     DisplayTask->Clear();
		     emb_string tmp ;
		 ...
		 if(stop_fl1)
		 {
		     ...
		     DisplayTask->StringOut(0,1,(uint8_t*)system_list + 11);
		     DisplayTask->StringOut(11,0,(uint8_t*)off_on + sys_param[auto_next_track] * 5);
		     DisplayTask->StringOut(11,1,(uint8_t*)up_down + sys_param[direction_counter] * 5);
		     DisplayTask->SymbolOut(15,1,62);
		     tim7_start(1);
		 }
	      }break;
...
//-----------------------------------Set Control MIDI-------------------------------------
	case midi_ctrl_menu:
		if(tim5_fl){
			if(!edit_fl)DisplayTask->Clear_str(0, 0 , 15);
			else {
				DisplayTask->StringOut(0, 0 , (uint8_t*)contrl_list + num_menu * 12);
				if(edit_fl == 1)DisplayTask->StringOut(0,1,(uint8_t*)"    ");
				else {
					if(edit_fl == 2)DisplayTask->StringOut(5,1,(uint8_t*)"    ");
				}
			}
			if(num_menu == 3)DisplayTask->StringOut(0, 1 , (uint8_t*)"Num ");
		}
		else {
			if(!edit_fl)DisplayTask->StringOut(0, 0 , (uint8_t*)contrl_list + num_menu * 12);
			else {
				if(edit_fl == 1)midi_type_print(ctrl_param[num_menu * 2]);
				else {
					if(edit_fl == 2)
					{
						if(ctrl_param[num_menu * 2])midi_note_print(ctrl_param[num_menu * 2 + 1]);
						else string_lin(5,1,ctrl_param[num_menu * 2 + 1] + 1);
					}
				}
			}
			if(num_menu == 3)
		...
		if(encoder_state1)
		{
			if (encoder_state == 1)
			{
				if(!edit_fl)
				{
					...
					{
						DisplayTask->StringOut(0, 0 , (uint8_t*)contrl_list + --num_menu * 12);
						if(num_menu < 3)(ctrl_param[num_menu * 2],ctrl_param[num_menu * 2 + 1]);
						else {
							DisplayTask->StringOut(0, 1 , (uint8_t*)"Num ");
							string_lin(5,1,ctrl_param[num_menu * 2 + 1] + 1);
						}
				...
				if(edit_fl == 1)
				{
					...
					if(temp)
					{
						...
						check_busy(temp,ctrl_param[num_menu * 2 + 1],num_menu + 1 ,0);
				...
				if(edit_fl == 2)
				{
					...
					if(temp)
					{
						if(num_menu != 3)
						{
							...
							check_busy(ctrl_param[num_menu * 2],temp,num_menu + 1 ,0);
						}
						else {
				...
				if(!edit_fl)
				{
					...
					{
						DisplayTask->StringOut(0, 0 , (uint8_t*)contrl_list + ++num_menu * 12);
						if(num_menu < 3)ev_midi_print(ctrl_param[num_menu * 2],ctrl_param[num_menu * 2 + 1]);
						else {
							if(num_menu < 4)
							{
								DisplayTask->StringOut(0, 1 , (uint8_t*)"Num ");
							...
							else DisplayTask->StringOut(8, 1 , (uint8_t*)"       ");
						}
						DisplayTask->StringOut(11,1,(uint8_t*)"    ");
					}
				}
				...
				if(edit_fl == 2)
				{
					...
					if(temp < 127)
					{
						if(num_menu < 3)
						{
							...
							if(ctrl_param[num_menu * 2 + 1] < 15)ctrl_param[num_menu * 2 + 1]++;
							DisplayTask->StringOut(0, 1 , (uint8_t*)"Num ");
							string_lin(5,1,ctrl_param[num_menu * 2 + 1] + 1);
						}
					}
		...
		if(key_ind == key_encoder)
		{
			edit_fl++;
			edit_fl %= 3;
			if(edit_fl == 1)if(num_menu == 3)edit_fl++;
			ev_midi_print(ctrl_param[num_menu * 2],ctrl_param[num_menu * 2 + 1]);
			if(num_menu == 3)
			{
				DisplayTask->StringOut(0, 1 , (uint8_t*)"Num ");
				string_lin(5,1,ctrl_param[num_menu * 2 + 1] + 1);
			...
			if(num_menu == 4)
			{
				...
				DisplayTask->StringOut(0,1,(uint8_t*)pc_list + pc_param[midi_pc * 2] * 5);
				string_lin(5,1,pc_param[midi_pc * 2 + 1] + 1);
				condish = midi_pc_menu;
			}
			if(!edit_fl)DisplayTask->StringOut(11,1,(uint8_t*)"    ");
			tim7_start(0);
		}
		if(key_ind == key_esc)
		{
...
//------------------------------------midi prog ch set------------------------------------
	case midi_pc_menu:
		if(tim5_fl){
			if(!edit_fl)DisplayTask->StringOut(7,0,(uint8_t*)"   ");
			else {
				if(edit_fl == 1)DisplayTask->StringOut(0,1,(uint8_t*)"    ");
				else {
					if(edit_fl == 2)DisplayTask->StringOut(5,1,(uint8_t*)"      ");
					else {
						if(pc_param[midi_pc * 2] != 2)string_lin(5,1,pc_param[midi_pc * 2 + 1] + 1);
						else midi_note_print(pc_param[midi_pc * 2 + 1]);
					}
				}
			}
		}
		else {
			if(!edit_fl)string_lin(7,0,midi_pc + 1);
			else {
				if(edit_fl == 1)DisplayTask->StringOut(0,1,(uint8_t*)pc_list + pc_param[midi_pc * 2] * 5);
			}
			if(pc_param[midi_pc * 2] != 2)string_lin(5,1,pc_param[midi_pc * 2 + 1] + 1);
			else midi_note_print(pc_param[midi_pc * 2 + 1]);
		}
		if(encoder_state1)
		{
			if (encoder_state == 1)
			{
				if(!edit_fl)
				{
					if(midi_pc)midi_pc = enc_speed_dec(midi_pc , 0);
					DisplayTask->StringOut(0,1,(uint8_t*)pc_list + pc_param[midi_pc * 2] * 5);
					if(pc_param[midi_pc * 2] != 2)string_lin(5,1,pc_param[midi_pc * 2 + 1] + 1);
					else midi_note_print(pc_param[midi_pc * 2 + 1]);
				}
				if(edit_fl == 1)
				{
					if(pc_param[midi_pc * 2])pc_param[midi_pc * 2]--;
				}
				if(edit_fl == 2)
				{
				...
				if(!edit_fl)
				{
					if(midi_pc < 98)midi_pc = enc_speed_inc(midi_pc, 98);
					DisplayTask->StringOut(0,1,(uint8_t*)pc_list + pc_param[midi_pc * 2] * 5);
					if(pc_param[midi_pc * 2] != 2)string_lin(5,1,pc_param[midi_pc * 2 + 1] + 1);
					else midi_note_print(pc_param[midi_pc * 2 + 1]);
				}
				if(edit_fl == 1)
				{
					if(pc_param[midi_pc * 2] < 2)pc_param[midi_pc * 2]++;
				}
				if(edit_fl == 2)
				{
					uint8_t temp = pc_param[midi_pc * 2 + 1];
		...
		if(key_ind == key_encoder)
		{
			edit_fl++;
			edit_fl %= 3;
			if(edit_fl == 1)string_lin(7,0,midi_pc + 1);
			if(edit_fl == 2)DisplayTask->StringOut(0,1,(uint8_t*)pc_list + pc_param[midi_pc * 2] * 5);
		}
		if(key_ind == key_esc)
		{
			DisplayTask->Clear();
...
//------------------------------------System----------------------------------------------
	case sys_menu:
	  if(tim5_fl)
	  {
	      if(!enc_key_fl)DisplayTask->Clear_str(0, num_menu & 1 , 10);
	      else DisplayTask->Clear_str(11, num_menu & 1 , 4);
	  }
	  else {
	      if(!enc_key_fl)DisplayTask->StringOut(0, num_menu & 1 , (uint8_t*)system_list + num_menu * 11);
	      else {
		  if(!num_menu || num_menu == 3)DisplayTask->StringOut(11, num_menu & 1 , (uint8_t*)off_on + sys_param[num_menu + auto_next_track] * 5);
		  else DisplayTask->StringOut(11, num_menu & 1 , (uint8_t*)up_down + sys_param[num_menu + auto_next_track] * 5);
	      }
	  }
	  if(encoder_state1)
	  {
		...
		if(!enc_key_fl)
		{
		   if(num_menu)
		   {
		       DisplayTask->StringOut(0, num_menu & 1 , (uint8_t*)system_list + num_menu-- * 11);
		       if(num_menu == 1)
			 {
			     DisplayTask->Clear();
			     DisplayTask->StringOut(0,0,(uint8_t*)system_list);
		    ...
		    {
			switch(num_menu){
		        case 0:DisplayTask->StringOut(11, 0 , (uint8_t*)off_on + --sys_param[auto_next_track] * 5);break;
			case 1:DisplayTask->StringOut(11, 1 , (uint8_t*)up_down + --sys_param[direction_counter] * 5);break;
			case 2:DisplayTask->StringOut(11, 0 , (uint8_t*)up_down + --sys_param[direction_scrol_playlist] * 5);break;
			case 3:DisplayTask->StringOut(11, 1 , (uint8_t*)off_on + --sys_param[loop_points] * 5);break;
			}
			tim7_start(1);
		    }
		}
		...
		if(!enc_key_fl)
		{
		   if(num_menu < 3)
		   {
		       DisplayTask->StringOut(0, 0 , (uint8_t*)system_list + num_menu++ * 11);
		       if(num_menu == 2)
			 {
			   DisplayTask->Clear();
			   DisplayTask->SymbolOut(15,0,60);
			   DisplayTask->StringOut(0, 1 , (uint8_t*)system_list + (num_menu + 1) * 11);
			   DisplayTask->StringOut(11, 0 , (uint8_t*)up_down + sys_param[direction_scrol_playlist]  * 5);
			   DisplayTask->StringOut(11, 1 , (uint8_t*)off_on + sys_param[loop_points]  * 5);
			 }
		       DisplayTask->StringOut(0, num_menu & 1 , (uint8_t*)system_list + num_menu * 11);
		       tim7_start(0);
		   }
	  ...
	  if(key_ind == key_encoder)
	  {
		 ...
		 enc_key_fl = 0;
		 if(!num_menu || num_menu == 3)DisplayTask->StringOut(11, num_menu & 1 , (uint8_t*)off_on + sys_param[num_menu] * 5);
		 else DisplayTask->StringOut(11, num_menu & 1 , (uint8_t*)up_down + sys_param[num_menu] * 5);
	      }
	      clean_fl();
	      tim7_start(0);
	  }
	...
	case edit_playlist:
	 ...
	 if(encoder_state1)
	 {
	    if (encoder_state==2)
	    {
	       ...
	       FsStreamTask->browser_name(tmp);
	       DisplayTask->Clear();
	       if(!num_tr_fl)DisplayTask->StringOut(0,0,(uint8_t*)"1:");
	       else DisplayTask->StringOut(0,0,(uint8_t*)"2:");
	       oem2winstar(tmp) ;
	       DisplayTask->StringOut(2,0,(uint8_t*)tmp.c_str());
	    }
	    else {
	       ...
	       if(!tmp.compare("/SONGS") && !tmp1.compare(".."))FsStreamTask->next_notify();
	       else {
	    	   DisplayTask->Clear();
	    	   if(!num_tr_fl)DisplayTask->StringOut(0,0,(uint8_t*)"1:");
	    	   else DisplayTask->StringOut(0,0,(uint8_t*)"2:");
	 ...
	 if(key_ind == key_encoder)
	 {
	  ...
	  if(!(!tmp.compare("/SONGS") && !tmp1.compare("..")))
	  {
	    if(!num_tr_fl)
	    {
		if(enc_dub_fl)play_next_file = 1;
		else play_next_file = 0;
	    }
	    FsStreamTask->action_notify( num_tr_fl ? TFsStreamTask::action_param_t::ap_2_wav : TFsStreamTask::action_param_t::ap_1_wav , num_prog_edit , 0);
	    if(act_fl)no_file = 0;
	    if(!act_fl && num_tr_fl)
	    {
	       DisplayTask->Clear();
	    ...
	    {
		if(play_next_file)DisplayTask->StringOut( 15 , 1 , (uint8_t*)">");
		else DisplayTask->Clear_str(15,1,1);
	    }
	    else DisplayTask->Clear_str(15,1,1);
	  }
	    clean_fl();
	...
	case select_folder:
	 if(encoder_state1)
	 {
	    if (encoder_state==2)
	    {
	       ...
	       FsStreamTask->browser_name(tmp1);
	       FsStreamTask->curr_path(tmp);
	       if(!tmp.compare("/PLAYLIST") && !tmp1.compare(".."))FsStreamTask->next_notify();
	       else {
		 DisplayTask->Clear();
		 oem2winstar(tmp1) ;
	  ...
	  if(encoder_state1)
	    {
	      if(encoder_state == 2)
		{
		  if(tempo < 240)tempo = enc_speed_inc(tempo, 240);
		  metronom_int = 44100.0f / (tempo / 60.0f) + 0.5f;
		  ind_temp();
		}
	      if(encoder_state == 1)
		{
		  if(tempo > 20)tempo = enc_speed_dec(tempo, 20);
		  metronom_int = 44100.0f / (tempo / 60.0f) + 0.5f;
		  ind_temp();
		}
	      clean_fl();
   ...
   if (tap_temp2 < 170000)
    {
      ...
      if (tap_temp1 < 150000)
       {
         if (tap_temp < 132300)tap_global = tap_temp;
       }
      else tap_global = 132300;
      a = 1;
    }
   tap_temp = 0;
   tap_temp1 = 0;
...
void ind_temp(void)
{
  ...
  if(tempo < 100)str[0] = 32;
  else str[0] = tempo / 100 + 48;
  str[2] = (tempo % 100);
  str[1] = str[2] / 10 + 48;
  str[2] = str[2] % 10 + 48;
