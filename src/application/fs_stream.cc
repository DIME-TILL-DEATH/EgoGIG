...
void TFsStreamTask::Code()
{
  ...
  DIR dir ;
	...
	while(1)
	  {
	    if(f_mkdir("/PLAYLIST") == FR_OK)break;
	  }
      }
	...
	while(1)
	  {
	    if(f_mkdir("/PLAYLIST/Default") == FR_OK)break;
	  }
      }
  ...
  if(f_open(&fsys,"/system.ego",FA_READ) == FR_OK)
  {
      ...
      if ( !sound_path.empty())browser.play_list_folder = sound_path.c_str();
  }
  else {
  ...
  if(f_open(&fsys,"/midi_map.ego",FA_READ) == FR_OK)
  {
      ...
      for(uint8_t i = 0 ; i < 128 ; i++)pc_param[i*2+1] = i;
...
bool TFsStreamTask::is_valid_wave(FIL* file , uint8_t num)
{
  wave_header_t wh ;
  UINT br = 0 ;
  if ( (fr = f_read(file, &wh, 12, &br )) != FR_OK ) return false;
  ...
  if ( wh.subchunk1Size != 16 &&  wh.subchunk1Size != 18) return false ;
  if ( wh.audioFormat != 1 ) return false ;
  if ( wh.numChannels != 2) return false ;
  if ( wh.sampleRate != 44100) return false ;
  if ( wh.bitsPerSample != 16) return false ;
  ...
  if(!num)
    {
      if ( (po_data % 6) ) { f_lseek(file, f_tell(file)+2) ; swap_need |= (swap_need_t)snSound ;}
      size = sz;
    }
...
void TFsStreamTask::action( action_param_t val , uint8_t play_index , uint8_t play_next)
        {
       	 ...
       	 else
       	 {
       		...
       		fr = f_closedir(&d);
       	     ...
       	     emb_string str2 ;
       	     char temp = 62;
             emb_printf::sprintf(str , "%s/%1.ego" , browser.play_list_folder.c_str(), play_index ) ;
        	 ...
        	 if(is_valid_wave(&f , 0))
        	 {
        	     f_close(&f) ;
            	     ...
            	     if(play_next_file)f_write(&f,&temp,1,fw);
            	     else f_lseek( &f , 0 );
            	     f_puts( str1.c_str() , &f );
            	     f_puts( str2.c_str() , &f );
            	     f_close(&f);
