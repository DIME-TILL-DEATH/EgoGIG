...
public:
  ...
  enum swap_need_t { snNo=0, snSound=1, snClick=2, snAll=3 } ;
  ...
  inline TFsStreamTask (const char* name , const int stack_size , const int priority ) : TTask(name , stack_size , priority , true)
     {

     }

      enum notify_t { qn_data = 0, qn_next, qn_prev,qn_curr,qn_owner,qn_action } ;
	...
	union
	{
     ...
     inline void data_notify(const target_t* val )
        {
          target = val ;
          query_notify_t qn = { .notify=qn_data };
          notify(qn) ;
         }
    ...
    inline void action_notify(action_param_t val, uint8_t play_index , uint8_t play_next)
        {
          query_notify_t qn ;
          qn.notify=qn_action;
          qn.action_param = val;
    ...
    inline void curr_notify()
        {
          query_notify_t qn = { .notify=qn_curr };
          notify(qn) ;
        }
   inline void owner_notify()
        {
          query_notify_t qn = { .notify=qn_owner };
          notify(qn) ;
        }
  ...
  inline error_t open_song_name(size_t index , emb_string& dst , uint8_t mode , uint8_t num)
     {
       ...
       if(fr != FR_OK)return eWaveNotPresent;
       uint8_t temp;
       f_read(&f_temp,&temp,1,fw);
       if(temp == 62)f_lseek(&f_temp,1);
       else f_lseek(&f_temp,0);
       f_gets(browser.buf,FF_MAX_SS,&f_temp);
       if(num)f_gets(browser.buf,FF_MAX_SS,&f_temp);
       f_close(&f_temp) ;
       dst = browser.buf;
       if(dst.empty())return eWaveNotPresent;
       if(!mode)dst = dst.substr( dst.find_last_of('/') + 1 , dst.length()) ;
  ...
  inline error_t open(size_t index) // выбор
     {
       ...
       emb_printf::sprintf(song , "%s/%1.ego" , browser.play_list_folder.c_str(), index ) ;
       ...
       if ( !sound_path.empty())
	 {
	   if ( (fr =  f_open(&file_sound, sound_path.c_str(), FA_READ)) != FR_OK ) return eFsError;
	   // проверка на валидность формата WAV файла A
           if (  !is_valid_wave(&file_sound , 0) )
	     {
           ...
           else sound_data_offset = file_sound.fptr;
       ...
       else
	 {
       ...
       if ( !click_path.empty())
       	 {
       	   if ( (fr = f_open(&file_click, click_path.c_str(), FA_READ)) != FR_OK ) return eFsError;
           ...
           else click_data_offset = file_click.fptr;
       	 }
...
*/
  ...
  inline void pos(size_t val)
     {
       fr = f_lseek (&file_sound, val*sizeof(wav_sample_t) + sound_data_offset);
       fr = f_lseek (&file_click, val*sizeof(wav_sample_t) + click_data_offset);
  ...
  inline bool play_list_folder()
     {
       if( (browser.fno.fattrib & AM_DIR) && (browser.fno.fname[0]!='.') && (browser.fno.fname[1]!='.') && (browser.fno.fname[1]!=0) ) //(!strcmp(browser.fno.fname,".."))
	 {
  ...
  inline void play_list_folder(emb_string& val ) const
     {
       val = browser.play_list_folder.substr( browser.play_list_folder.find_last_of('/')+1 , browser.play_list_folder.length()) ;
     }
  ...
  {
    if ( browser.fno.fattrib & AM_DIR )return false;
    else return true;
  }
  ...
  inline bool SoundNeedSwap() { return swap_need & snSound ; }
  inline bool ClickNeedSwap() { return swap_need & snClick ; }


  inline void MidiEventProcess() {  midi_player.process(); }
...
protected:
     ...
     inline void get()
     {
       ...
       else [[likely]] sound_strech( target ) ;


       // чтение данных из файла click и запись в буффер
       br = 0 ;
       if((file_click.fptr) > samp_point)samp_point = file_click.fptr;
       ...
       else [[likely]] click_strech( target ) ;
          ...
          inline void notify(const query_notify_t& val)
            {
              if ( cortex_isr_num())
         	{
         	   ...
         	   BaseType_t xHigherPriorityTaskWoken ;
                   NotifyFromISR( *((uint32_t*)&val), eSetValueWithOverwrite,&xHigherPriorityTaskWoken);
