...
extern "C" void USART1_IRQHandler()
{
	...
	case 0:
		...
		if (us_buf == 0xfc)
		{
			CSTask->notify(TCSTask::qn_stop_song);
			midi_buf = 0;
		}
	...
	case 0xb0: //CC
		...
		if(player.state() == Player::PLAYER_IDLE)
		{
			for (uint8_t i = 0; i < 99; i++)
			{
				...
				{
					CSTask->notify(TCSTask::qn_load_song, us_buf);
					break;
				}
			}
		}
