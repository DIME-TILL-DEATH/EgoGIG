  ...
  while(1)
    {
      ...
      switch(cmd.cmd)
      {
         case dcClear :
           ...
           break;
         case dcSymbolOut:
           lcd44780_ShowChar(cmd.SymbolOutParams.x , cmd.SymbolOutParams.y , cmd.SymbolOutParams.symbol);
           break ;
         case dcStringOut:
           lcd44780_ShowStr(cmd.StringOutParams.x , cmd.StringOutParams.y , cmd.StringOutParams.string);
           break ;
         case dcClear_str:
           lcd44780_Clear_str(cmd.Clear_strParams.x , cmd.Clear_strParams.y , cmd.Clear_strParams.count);
           break ;
         case dcCurPos:
...
void TDisplayTask::StringOut(uint8_t x, uint8_t y , uint8_t* string)
{
  ...
  strncpy ( (char*)cmd.StringOutParams.string , (const char*)string , FILE_NAME_LENGTH ) ;
  Command( &cmd ) ;
}
