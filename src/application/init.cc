...
const target_t first_target  = {  (char*)sound_buff ,                     (char*)click_buff ,                     wav_buff_size / 2 * sizeof (wav_sample_t) } ;
const target_t second_target = {  (char*)(sound_buff + wav_buff_size/2) , (char*)(click_buff + wav_buff_size/2) , wav_buff_size / 2 * sizeof (wav_sample_t) } ;

volatile uint8_t encoder_state,encoder_state1,encoder_key,key_ind,play_fl,play_fl1;
...
void init(void)
{
		  ...
		  gpio_mode_setup(GPIOB, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO3 | GPIO4 | GPIO5 | GPIO10 | GPIO12 | GPIO15 | GPIO6 | GPIO7); // I2S3 CK | SPI1 MISO | I2S3 SD
		  gpio_set_output_options(GPIOB, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO3 | GPIO4 | GPIO5 | GPIO10 | GPIO12 | GPIO15 | GPIO6 | GPIO7); // I2S2 CK | I2S2 WS   | I2S2 SD | USART1 TX | USART1 RX
		  ...
		  gpio_mode_setup(GPIOC, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO7 | GPIO14 | GPIO15);  // DAC PWD | LED CS | KEY CS
		  gpio_set_output_options(GPIOC, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO7 | GPIO14 | GPIO15);
	...
	//-----------------------------------------------------------------------------------------------------------------
		  ...
		  SPI2_I2SCFGR = SPI_I2SCFGR_CHLEN | SPI_I2SCFGR_DATLEN_1 | SPI_I2SCFGR_I2SCFG_1 | SPI_I2SCFGR_I2SMOD;
		  ...
		  SPI3_I2SCFGR = SPI_I2SCFGR_CHLEN | SPI_I2SCFGR_DATLEN_1 | SPI_I2SCFGR_I2SMOD;
		  //SPI3ext_I2SCFGR = SPI_I2SCFGR_CHLEN | SPI_I2SCFGR_DATLEN_1 | SPI_I2SCFGR_I2SCFG_0 | SPI_I2SCFGR_I2SMOD;
		  ...
		  for(uint32_t i = 0 ; i < 0x3fffff ; i++)NOP();

		  gpio_set(GPIOC,GPIO7);
		  for(int i = 0 ; i < 0xfffff ; i++)NOP();
		  const uint16_t fff[2] = {0x2087,0xa103};
		  for(int i = 0 ; i < 1 ; i++)
		   {
		  ...
		  for(uint16_t i = 0 ; i < 0xffff ; i++)NOP();
		  ...
		  usart_set_flow_control(USART1, USART_FLOWCONTROL_NONE);
 		  ...
 		  USART1_BRR = (uint16_t)(((2 * clock_pll) + baudrate_midi) / (2 * baudrate_midi));
		  ...
		  for(int i = 0 ; i < 0xff ; i++)NOP();
		  //---------------------------------------------------------bypass unselect pin---------------------------------------------
		  gpio_mode_setup(GPIOA, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO8 | GPIO11 | GPIO12 /*| GPIO13 | GPIO14*/);
		  gpio_set_output_options(GPIOA, GPIO_OTYPE_PP, GPIO_OSPEED_2MHZ, GPIO6 | GPIO8 | GPIO11 | GPIO12 | GPIO13 | GPIO14);
...
extern "C" void DMA1_Stream4_IRQHandler()
{
  static size_t i = 0 ;
	...
	if(count_up >= song_size)
	{
           play_fl = 0;
           stop_fl1 = 1;
	   if(FsStreamTask->next_pl())condish = play_next_fil;
	   else {
	     if(!sys_param[0])key_ind = key_stop;
	     else key_ind = key_right_down;
	   }
	   CSTask->Give();
	}
	else {
	    if(sound_point == 0)FsStreamTask->data_notify( &second_target ) ;
	    if(sound_point == wav_buff_size/2)FsStreamTask->data_notify( &first_target ) ;
	}
	 ...
	 if(msec_tik == 4410)
	 {
	     ...
	     if(!condish)
	     {
		 if(sys_param[1])DisplayTask->Sec_Print(count_down);
	     ...
	     {
	    	 if(play_point2 > play_point1)if((count_up * 4410) >= play_point2)
	    	 {
	    		 key_ind = key_return;
	    		 CSTask->Give();
	    	 }
	    	 if(play_point1 > play_point2)if((count_up * 4410) >= play_point1)
	    	 {
	    		 key_ind = key_forward;
	    		 CSTask->Give();
	    	 }
	...
	if(!temp_counter++)metronom_fl = 1;
	else {
	    if(temp_counter == metronom_int)temp_counter = 0;
	}
      }
    if (tap_temp < 132300)tap_temp++;
    if (tap_temp1 < 150001)tap_temp1++;
    if (tap_temp2 < 170001)tap_temp2++;
}
...
extern "C" void EXTI4_IRQHandler()
{
  ...
  if(!lock_fl)
  {
    if(drebezg(EXTI4) == 1)
    {
	if(GPIOC_IDR & GPIO5)encoder_state = 1;
...
extern "C" void DMA2_Stream0_IRQHandler()
{
  ...
  if(!gpio_get(GPIOC,GPIO0))key_val = 1;
  else {
      if(!gpio_get(GPIOB,GPIO11))key_val = 2;
      else {
	  if(!gpio_get(GPIOB,GPIO13))key_val = 3;
	  else {
	      if(!gpio_get(GPIOB,GPIO14))key_val = 4;
	      else key_val = key_reg_in[1] >> 8;
	  }
      }
  }
